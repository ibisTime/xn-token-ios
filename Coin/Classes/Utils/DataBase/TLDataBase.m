...
- (void)createTable
{
    ...
    if(![fileManager fileExistsAtPath:_dataStr]) {
        ...
        if([db open]) {
            ...
            [db executeUpdate:@"create table if not exists THAWallet(walletId INTEGER PRIMARY KEY AUTOINCREMENT,userId text, Mnemonics text, wanaddress text,wanprivate text,ethaddress text,ethprivate text,btcaddress text,btcprivate text,PwdKey text)"];
