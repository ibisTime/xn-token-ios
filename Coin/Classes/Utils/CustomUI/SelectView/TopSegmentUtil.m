...
-(NSArray *)observeKeyPaths{
    NSArray *pathArr = [NSArray arrayWithObjects:@"titleNormalColor",@"titleSelectColor",@"titleFont",@"defaultSelectIndex", @"lineType",@"bgNormalColor",@"bgSelectColor", nil];
    return pathArr;
}
...
-(void)updateUIWithNewValueOfKeypath:(NSString *)keyPath{
    if ([keyPath isEqualToString:@"titleNormalColor"]) {
        ...
        [self updateSegmentViewUI:^(UIButton *btn) {
            [btn setBackgroundColor:self.bgSelectColor forState:UIControlStateSelected];
        }];
...
//    CGFloat lineW = [NSString getWidthWithString:self.titleArray[0] font:self.titleFont.lineHeight];
    ...
    self.lineWidth = self.width / titleArray.count;
    ...
    for (int i = 0; i<_titleArray.count; i++) {
        ...
        btn.frame = CGRectMake(self.segmentWidth/4.0-BTN_LINE_WIDTH/2.0+self.segmentWidth/2.0*i, 0, BTN_LINE_WIDTH, self.segmentHeight);
        btn.tag = i+1000;
