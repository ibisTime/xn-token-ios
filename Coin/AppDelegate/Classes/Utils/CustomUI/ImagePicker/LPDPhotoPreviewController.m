...
- (void)select:(UIButton *)selectButton {
    ...
    if (!selectButton.isSelected) {
        ...
        if (_lpdImagePickerVc.selectedModels.count >= _lpdImagePickerVc.maxImagesCount) {
            NSString *title = [NSString stringWithFormat:[NSBundle lpd_localizedStringForKey:[LangSwitcher switchLang:@"你最多只能选择9张图片" key:nil]], _lpdImagePickerVc.maxImagesCount];
