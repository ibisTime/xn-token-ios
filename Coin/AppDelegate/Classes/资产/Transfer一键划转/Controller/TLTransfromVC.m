...
//        _tableView.models = _localcurrencys;
        _tableView.model = _localcurrencys[0];
...
//            self.currentModel = _centercurrencys[index];
            self.numberTextField.text = @"";
        ...
        if (index == 101) {
            NSString *leftAmount = [self.tableView.model.amountString subNumber:self.tableView.model.frozenAmountString];
            NSString *money = [CoinUtil convertToRealCoin:leftAmount coin:self.tableView.model.symbol];
            self.numberTextField.text = money;
            [self.tableView reloadData];
        }
        //    划转
...
//    layout.itemSize = CGSizeMake((SCREEN_WIDTH - 60)/3, 42);
...
//    float numberToRound;
//    int result;
//    numberToRound = (self.currencys.count + 1.0)/3.0;
//    result = (int)ceilf(numberToRound);
//
//    THTranstionCollectionView *nineView = [[THTranstionCollectionView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 20 + result * (42 + 20)) collectionViewLayout:layout];
...
////        make.height.equalTo(@(15 + result * (42 + 10)));
////    }];
